...
import { IncomeAPI, CategoryAPI, FileAPI } from "../api/api";

export default function EditIncomeModal({ isOpen, onClose, item, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen) {
      CategoryAPI.list()
        .then((res) => {
          const incomeCats = res.categories.filter((c) => c.type_of_category === "income");
  ...
  useEffect(() => {
    if (item) {
      const dateStr = item.income_date || item.date || "";
      setForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (item?.id) {
        ...
        if (selectedFile) {
          await FileAPI.upload(`income_${item.id}`, selectedFile);
        }
      } else {
        const fd = new FormData();
        fd.append("income_date", dateTime);
        fd.append("category_id", String(parseInt(form.category_id, 10)));
        fd.append("value", String(parseFloat(form.amount)));
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-3xl p-6 w-[400px] shadow-lg flex flex-col gap-3">
        <h2 className="text-center text-lg font-semibold">
          {item ? "Редактировать доход" : "Добавить доход"}
        </h2>
